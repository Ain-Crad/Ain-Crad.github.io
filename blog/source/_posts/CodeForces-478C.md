...
$取两个$a[2]$组成三气球或者每取一个$a[1]$取两个$a[2]$组成三气球,即, 取球集合为$(1, 0, 2)$和$(0, 1, 2)$.答案为$a[0] + a[1]$.
* $2 * (a[0] + a[1]) > a[2]$.这种情况下我们从$a[2]$中取两个气球同时从$max(a[0], a[1])$中取一个气球, 直到$a[2] <= max(a[0], a[1])$.这时满足$max(a[0], a[1]) - a[2] <= 1, max(a[0], a[1]) - min(a[0], a[1]) <= 1, max(a[0], a[1], a[2]) - min(a[0], a[1], a[2]) <= 1$. 接下来的按集合$(1, 1, 1)$继续取球.**可以发现这种取法最后剩余球的数量一定是**$(a[0]+a[1]+a[2]) % 3$**除去剩余的这些球(数量小于3), 其它球每三个装饰一张桌子, 所以针对这种情况我们直接可以用**$(a[0]+a[1]+a[2]) / 3$**来得到最终结果**
